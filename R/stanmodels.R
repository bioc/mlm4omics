# Part of the mlm4omics package for estimating model parameters
...
# MA  02110-1301, USA.
# This file is only intended to be used during the installation
#process
# nocov start
MODELS_HOME = "src"
if (!file.exists(MODELS_HOME)) MODELS_HOME = sub("R$", "src",
getwd())
...
stanmodels = lapply(stan_files, function(f) {
    ...
    return(do.call(methods::new, args =
        c(stanfit[-seq_len(3)],
        Class = "stanmodel", 
        mk_cppmodule = function(x)
        get(paste0("model_", model_cppname)))))
    }
)
